...
select * from Score where s_id = '08';
select * from Student where s_id not  in (select s_id
...
select  Student.s_id,Student.s_name from (select distinct s_id from  Score where c_id in (select c_id from Score where s_id = '01')) as s
    inner join  Student
